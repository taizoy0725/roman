...
最終確認: 2026-08-10(v1.0 の配布物 — `native/ime/release.sh` が生成する `Roman-1.0.pkg` の実同梱物 — に基づく。Llama-3-ELYZA-JP-8B は削除済み)。
...
| --- | --- | --- | --- | --- | --- |
| Roman 本体(入力メソッド・変換サーバ・辞書構築物) | 1.0 | — | `Roman.app` / `server/` | 作者が定める利用条件(All rights reserved) | 利用条件は [TERMS.md](../TERMS.md) |
...
| llama.cpp (llama-server と共有ライブラリ) | b9960 | zenz の推論ランタイム | `runtime/llama.cpp/` | MIT | 著作権表示とライセンス文の保持 → 上流の原文を `runtime/llama.cpp/LICENSE` としてそのまま同梱 |
...
Roman はこれらをローカルの HTTP エンドポイントとして参照するだけで、配布物には一切含めない。 したがって**再配布に伴う義務は生じない**が、何に依存しているかの記録として残す。

| コンポーネント | 役割 | 出自 | ライセンス |
| --- | --- | --- | --- |
| ollama | 汎用 LLM のランタイム | [ollama/ollama](https://github.com/ollama/ollama) | MIT |
| Qwen2.5-7B-Instruct | 候補の裁定 LLM(僅差の文節解を全文で選び直す) | [Qwen/Qwen2.5-7B-Instruct](https://huggingface.co/Qwen/Qwen2.5-7B-Instruct) | Apache 2.0(Qwen2.5 系は 3B/72B のみ独自ライセンス。7B は Apache 2.0 確認済み) |
...
| PyTorch / Transformers / fugashi / unidic-lite | BERT の推論ランタイム(`~/.roman-bert/venv`) | PyPI | BSD-3 / Apache 2.0 / BSD ほか |
...
- **利用者環境のコンポーネント (ollama・Qwen・BERT・PyTorch) について**: 再配布していないため義務は生じない。将来これらを同梱する設計に変えた場合は、 それぞれの LICENSE / NOTICE の同梱が新たに必要になる
- **アップデート時に見直す箇所**: llama.cpp のビルド番号・Node.js のバージョンを 上げたときは、`release.sh` の sha256 と併せて本ページの版表記も更新する

## 履行状況のチェックリスト

- [x] 推論ランタイムの整理 — zenz は同梱の llama-server で動かす。裁定用の汎用 LLM は 利用者環境の ollama を使い、同梱しない
- [x] Qwen (4.7GB) は同梱せず、利用者環境の ollama から利用する
...
### v1.0 (2026-08-10 出荷分) の未履行

出荷済みの `Roman-1.0.pkg` は Markdown 化前の版で、`runtime/NOTICE.html` に **zenz の帰属表示を含んでいない**(導入済み環境の実物で確認済み)。 CC BY-SA 4.0 第3条(a)(1) を満たしていないため、**次の配布で必ず解消する**こと。 上記3項目はいずれもその是正である。
