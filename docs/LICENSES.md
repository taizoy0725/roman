...
| 法律用語リスト(SKK-JISYO.law 由来) | 上流コミット f415e667 (2019-04-27) | 法律用語の変換 | `server/dict/law-skk.tsv` | GNU GPL v2 以降 | 条文全文の同梱・著作権表示・改変の明示 → [NOTICE.md](NOTICE.md) に条文全文と帰属表示、改変日を掲載。GPL は本ファイル自体に適用され (GPL のまま配布)、実行時に読み込むデータであってリンクされないため、他の同梱物へは及ばない |
